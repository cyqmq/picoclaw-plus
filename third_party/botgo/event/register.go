...
var DefaultHandlers struct {
	...
	GroupManage        GroupManageEventHandler
	C2CManage          C2CManageEventHandler
...
// GroupManageEventHandler 群管理事件 handler（机器人进退群、主动消息权限变更）
type GroupManageEventHandler func(event *dto.WSPayload, data *dto.WSGroupManageData) error

// C2CManageEventHandler C2C 管理事件 handler（主动消息权限变更）
type C2CManageEventHandler func(event *dto.WSPayload, data *dto.WSC2CManageData) error
...
func RegisterHandlers(handlers ...interface{}) dto.Intent {
	...
	for _, h := range handlers {
		...
		case GroupManageEventHandler:
			DefaultHandlers.GroupManage = handle
			i = i | dto.EventToIntent(
				dto.EventGroupAddRobot, dto.EventGroupDelRobot,
				dto.EventGroupMsgReject, dto.EventGroupMsgReceive,
			)
		case C2CManageEventHandler:
			DefaultHandlers.C2CManage = handle
			i = i | dto.EventToIntent(
				dto.EventC2CMsgReject, dto.EventC2CMsgReceive,
			)
